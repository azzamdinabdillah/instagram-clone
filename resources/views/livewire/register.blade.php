<div class="bg-white">
        ...
        <div class="w-full py-10 flex justify-center items-center xl:bg-putih-gelap-dikit px-5 flex-row-reverse">
            <div class="xl:w-[25%] w-[80%]">
                <div class="xl:bg-white xl:border-2 xl:rounded xl:p-7">
                    <h1 class="text-4xl font-bold text-center mb-4 xl:mb-7">Instagram</h1>

                    <p class="font-semibold text-lg text-slate-500 text-center mb-5">Sign up to see photos and videos from your friends.</p>
                    ...
                    <div class="block w-[100%]">
                        <form wire:submit.prevent="registerSystem" class="w-full">
                            <div class="relative form-login">
                                <input autocomplete="off" type="text" wire:model="email" id="number" class="bg-[#FAFAFA] block px-2 pb-2 pt-6 w-full border-2 placeholder:text-xs rounded transition text-xs" required="required">
                                <label for="number" class="absolute top-1/2 -translate-y-1/2 left-2 text-xs text-slate-400 label transition-all">Mobile Number or Email</label>
                            </div>
                            @error('email')
                            <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                            @enderror
                            <div class="relative form-login">
                                <input autocomplete="off" type="text" wire:model="name" id="name" class="bg-[#FAFAFA] block px-2 pb-2 pt-6 mt-3 w-full border-2 placeholder:text-xs rounded transition text-xs" required="required">
                                <label for="name" class="absolute top-1/2 -translate-y-1/2 left-2 text-xs text-slate-400 label transition-all">Full Name</label>
                            </div>
                            @error('name')
                            <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                            @enderror
                            <div class="relative form-login">
                                <input autocomplete="off" type="text" wire:model="username" id="username" class="bg-[#FAFAFA] block px-2 pb-2 pt-6 mt-3 w-full border-2 placeholder:text-xs rounded transition text-xs" required="required">
                            ...
                            <div class="relative form-login">
                                ...
                                @if ($showPw == true)
                                    <input autocomplete="off" wire:model="password" type="text" id="password" class="bg-[#FAFAFA] block px-2 pb-2 pt-6 mt-3 w-full border-2 placeholder:text-xs rounded transition text-xs" required="required">
                                @else
                                    <input autocomplete="off" wire:model="password" type="password" id="password" class="bg-[#FAFAFA] block px-2 pb-2 pt-6 mt-3 w-full border-2 placeholder:text-xs rounded transition text-xs" required="required">
                                @endif
                            ...
                            <p class="text-slate-500 text-xs text-center mt-4">People who use our service may have uploaded your contact information to Instagram. Learn More</p>

                            <p class="text-slate-500 text-xs text-center mt-4">By signing up, you agree to our Terms , Privacy Policy and Cookies Policy .</p>

                            <button type="submit" class="w-full py-1 bg-blue-500 mt-5 xl:mt-5 font-semibold text-white rounded">Register</button>

                            <p class="mt-7 block xl:hidden text-slate-400 text-sm text-center">Have an account? <a href="/login" class="text-blue-500">Sign up</a></p>
                        </form>
                ...
                <div class="hidden xl:block mt-3 w-full">
                    <p class="py-5 bg-white border-2 rounded block text-slate-400 text-sm text-center">Have an account? <a href="/login" class="text-blue-500">Sign up</a></p>
                </div>
